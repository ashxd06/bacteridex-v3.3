...
import { CATEGORIAS_ANALISIS, analisisClinicos as analisisJSON } from "@/lib/data";
import { getAnalisisPorId, getProcedimientosLab, getCalculadoraPorAnalisisId } from "@/lib/content";
import InsertosRelacionados from "@/components/analisis/InsertosRelacionados";

export function generateStaticParams() {
  return analisisJSON.map((a) => ({ id: a.id }));
}
...
export default async function Page({ params }: { params: { id: string } }) {
  const analisis = await getAnalisisPorId(params.id);
  if (!analisis) return notFound();

  const procedimientos = await getProcedimientosLab();
  const calculadora = await getCalculadoraPorAnalisisId(analisis.id);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <InsertosRelacionados analisisId={analisis.id} />

      {calculadora && (
        <Link href={`/calculadoras/${calculadora.id}`} className="lab-card focus-ring flex items-center justify-between p-4">
          <span className="text-sm font-medium">🧮 Calculadora: {calculadora.nombre}</span>
          <span className="text-xs text-bio">Calcular →</span>
        </Link>
      )}
