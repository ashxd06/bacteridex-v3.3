...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { error } = await verificacion.admin.from("cms_procedimientos").update(cambios).eq("id", params.id);
  if (error) {
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { error } = await verificacion.admin.from("cms_procedimientos").delete().eq("id", params.id);
  if (error) {
