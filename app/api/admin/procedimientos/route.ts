...
export async function GET(req: NextRequest) {
  ...
  if (q) {
    filas = filas.filter((fila) => {
      const p = fila.data as Procedimiento;
      const texto = [p.nombre, p.objetivo, fila.id].join(" ").toLowerCase();
...
export async function POST(req: NextRequest) {
  ...
  const data = body?.data as Procedimiento | undefined;
  ...
  const { error } = await verificacion.admin.from("cms_procedimientos").insert({
    id: data.id,
  ...
  if (error) {
    ...
    const mensaje = error.code === "23505" ? "Ya existe un procedimiento con ese id." : "No se pudo crear el registro.";
