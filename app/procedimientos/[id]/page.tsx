...
import { procedimientos as procedimientosJSON, CATEGORIAS_PROCEDIMIENTOS } from "@/lib/data";
import { getProcedimientoPorId, getTodosLosMicroorganismos } from "@/lib/content";
import { imagenesProcedimientos, getImagen } from "@/lib/images";
import ImagePlaceholder from "@/components/ImagePlaceholder";

export function generateStaticParams() {
  return procedimientosJSON.map((p) => ({ id: p.id }));
}
...
export default async function Page({ params }: { params: { id: string } }) {
  const proc = await getProcedimientoPorId(params.id);
  if (!proc) return notFound();

  const todosLosOrganismos = await getTodosLosMicroorganismos();
