...
import { imagenesMicroscopica, imagenesAgar, getImagen, candidatosMicroscopica, candidatosAgar } from "@/lib/images";
...
export default function OrganismDetail({ organismo }: { organismo: Organismo }) {
  ...
  return (
    <div className="flex flex-col gap-8">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div onClick={() => marcar("reconocioImagen")} className="cursor-pointer">
            <ImagePlaceholder
              ...
              candidatos={candidatosMicroscopica(organismo.nombreCientifico)}
              onAbrir={setImagenAmpliada}
            />
          ...
          <div onClick={() => marcar("reconocioImagen")} className="cursor-pointer">
            <ImagePlaceholder
              ...
              url={imagenAgar}
              candidatos={candidatosAgar(organismo.nombreCientifico)}
              onAbrir={setImagenAmpliada}
            />
      ...
      {imagenAmpliada && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-base-950/90 p-4 backdrop-blur-sm"
          onClick={() => setImagenAmpliada(null)}
        >
          <button
            onClick={() => setImagenAmpliada(null)}
            aria-label="Cerrar"
            className="focus-ring absolute right-4 top-4 text-2xl text-mist-100 hover:text-bio"
          >
            ✕
          </button>
          ...
          <img
            src={imagenAmpliada}
            alt={organismo.nombreCientifico}
            className="max-h-[85vh] max-w-full rounded-2xl border border-base-600 object-contain"
