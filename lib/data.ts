...
export const analisisClinicos = analisisData as AnalisisClinico[];

export const CATEGORIAS_ANALISIS: { id: CategoriaAnalisis; label: string; emoji: string }[] = [
  ...
  { id: "uroanalisis", label: "Uroanálisis", emoji: "🧪" },
  { id: "serologia", label: "Serología", emoji: "🧫" },
  { id: "otras", label: "Otras áreas", emoji: "🔬" }
];

export function getAnalisis(id: string): AnalisisClinico | undefined {
  return analisisClinicos.find((a) => a.id === id);
}
...
export interface ResultadoBusqueda {
  tipo: "organismo" | "prueba" | "medio" | "procedimiento" | "analisis";
  id: string;
  categoria?: Categoria;
  titulo: string;
  subtitulo: string;
...
export function buscarGlobal(query: string): ResultadoBusqueda[] {
  ...
  for (const a of analisisClinicos) {
    const campos = [a.nombre, a.descripcion, a.utilidad, a.metodo, a.tipoMuestra]
      .join(" ")
      .toLowerCase();
    if (campos.includes(q)) {
      resultados.push({
        ...
        id: a.id,
        titulo: a.nombre,
        subtitulo: `Análisis clínico · ${a.categoria}`
      });
    }
  }
